...
mkRecConst :: R5ClassDef -> (Variable, Type)
mkRecConst (ClassDef name super fields methods) =
  let fieldNames = map fst fields
      fieldTypes = map snd fields
  in (name, FunT fieldTypes (ClassNameT name))
...
typecheck (records, defns, e) =
  let defTypeEnv = map getDefnType defns
      ...
      recordsEnv = map (\(ClassDef name super fields methods) ->
                          (name, ClassDef name super fields methods)) records
...
shrink :: TypedR5Program -> ([R5ClassDef], [TypedR5Definition])
...
uniquify :: ([R5ClassDef], [TypedR5Definition]) -> ([R5ClassDef], [TypedR5Definition])
uniquify (records, defns) =
  let initEnv  = map (\(DefnT name _ _ _) -> (name, name)) defns
      recEnv   = map (\(ClassDef name _ _ _) -> (name, name)) records
...
crecClass :: R5ClassDef -> TypedR5Definition
crecClass (ClassDef name super fields methods) =
  let fieldNames = map fst fields
      fieldTypes = map snd fields
      fieldNameVars = map (\(x, t) -> VarTE x t) fields
      body = VectorTE fieldNameVars (VectorT fieldTypes)
  in DefnT name fields (ClassNameT name) body

convertClasses :: ([R5ClassDef], [TypedR5Definition]) -> [TypedR5Definition]
convertClasses (records, defns) = (map crecClass records) ++ (map crecDefn defns)
...
compile = printX86 . patchInstructions . allocateRegisters . buildInterference .
  ...
  limitFunctions . revealFunctions . convertClasses . uniquify .
  shrink . typecheck
...
compileLog e =
  ...
  (logOutput "convertClasses" convertClasses) >>=
