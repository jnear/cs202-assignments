...
getFieldExpr :: Parser (R5Expr -> R5Expr)
getFieldExpr = do
  reservedOp "."
  field <- identifier
  return (\e -> GetFieldE e field)

-- Operator table
table :: Ex.OperatorTable String () Identity R5Expr
table   = [ [ Ex.Postfix funCallExpr ]
          , [ Ex.Postfix getFieldExpr ]
          , [ prefixOp "not" NotE ]
...
pClassType :: Parser Type
pClassType = do
  id <- identifier
  return $ ClassNameT id
...
classDef :: Parser R5ClassDef
classDef = do
  reserved "class"
  name <- identifier
  reserved "extends"
  super <- identifier
  (fields, methods) <- braces $ do
    f <- commaSep typedArg
    m <- many def
    return (f, m)
  return $ ClassDef name super fields methods
...
program = do
  classDefs <- many $ classDef
  defs <- many $ def
  e <- expr
  return (classDefs, defs, e)
