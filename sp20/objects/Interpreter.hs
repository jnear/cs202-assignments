...
data Val = IntVal Int | BoolVal Bool | AddrVal Addr | VoidVal
         ...
         | ClassConstructor String [(String, Type)]
         | ClassVal String [(String, Val)]
...
eval e env sto = case e of
  ...
  FunCallE f args ->
    case eval f env sto of
      ...
      (ClassConstructor name fields, sto1) ->
        let (vals, sto2) = evalArgs args env sto1
            fieldNames   = map fst fields
        in (ClassVal name (zip fieldNames vals), sto2)
  LambdaE args body -> (ClosureVal (map fst args) body env, sto)
  GetFieldE e1 f ->
    let (ClassVal _ fields, sto1) = eval e1 env sto
...
buildClassEnv :: [R5ClassDef] -> Env
buildClassEnv records = map mkClassConstructor records

mkClassConstructor :: R5ClassDef -> (String, Val)
mkClassConstructor (ClassDef name super fields methods) = (name, ClassConstructor name fields)

evalProgram :: R5Program -> (Val, Store)
evalProgram (records, defs, e) =
  let recordEnv = buildClassEnv records
