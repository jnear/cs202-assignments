...
-- Typechecker for R4 Expressions
-- Inputs:
--  - e: an R4 expression
--  - env: a type environment mapping variables to their types
-- Output: a pair
--  - the type of e, if it is well-typed
--  - a *typed* R4 expression
-- Throws a run-time error if e is not well-typed
tcExpr :: R4Expr -> TEnv -> (Type, TypedR4Expr)
tcExpr e env = undefined

-- Get the name and type of a definition
getDefnType :: R4Definition -> (Variable, Type)
getDefnType (Defn name args outputType body) =
  (name, FunT (map snd args) outputType)

-- Typechecker for R4 definitions
tcDefn :: TEnv -> R4Definition -> TypedR4Definition
tcDefn env (Defn name args outputT body) = undefined

-- Typechecker for R4 programs
typecheck :: R4Program -> TypedR4Program
typecheck (defns, e) =
...
-- Shrink, for R4 expressions
-- Input: e, an R4 expression
-- Output: an R4 expression
-- Removes "and", "or", ">=", ">", ">="
shrinkExpr :: TypedR4Expr -> TypedR4Expr
shrinkExpr e = undefined

-- The shrink pass, for an R4 definition
-- Input:  an R4 definition
-- Output: an R4 definition
shrinkDefn :: TypedR4Definition -> TypedR4Definition
shrinkDefn (DefnT name args outputT body) =
  (DefnT name args outputT body)

-- The shrink pass, for an R4 program
-- Input:  an R4 program
-- Output: a list of R4 definitions
-- Moves the top-level expression into a "main" definition
shrink :: TypedR4Program -> [TypedR4Definition]
shrink (defns, e) =
...
-- Output: an R4 expression, with variables renamed to be unique
uniquifyExp :: TypedR4Expr -> VEnv -> TypedR4Expr
uniquifyExp e env = undefined

-- The uniquify pass, for a single R4 definition
-- Input:  an R4 definition
-- Output: an R4 definition, with variables renamed to be unique
uniquifyDefn :: VEnv -> TypedR4Definition -> TypedR4Definition
uniquifyDefn initEnv (DefnT name args outputT body) = undefined

-- The uniquify pass, for a list of R4 definitions
-- Input:  a list of R4 definitions
-- Output: a list of R4 definitions, with variables renamed to be unique
uniquify :: [TypedR4Definition] -> [TypedR4Definition]
uniquify defns =
...
-- Reveal-functions, for R4 expressions
-- Input: e, an R4 expression
-- Output: an R4 expression
-- Transforms variables referencing function names with FunRefTE expressions
revealExpr :: TypedR4Expr -> [String] -> TypedR4Expr
...
-- Output: an R4 expression
-- Transforms variables referencing function names with FunRefTE expressions
revealDefn :: [String] -> TypedR4Definition -> TypedR4Definition
revealDefn funs (DefnT name args outputT body) =
  (DefnT name args outputT (revealExpr body funs))

-- Reveal-functions, for a list of R4 definitions
-- Input: a list of R4 definitions
-- Output: a list of R4 definitions
-- Transforms variables referencing function names with FunRefTE expressions
revealFunctions :: [TypedR4Definition] -> [TypedR4Definition]
...
type EEnv = [(Variable, TypedR4Expr)]

-- Limit-functions, for R4 expressions
-- Input:
--  - e, an R4 expression
--  - env, an environment
-- Output: an R4 expression
-- Moves arguments > 6 into a vector
limitExpr :: TypedR4Expr -> EEnv -> TypedR4Expr
limitExpr e env = undefined

-- Limit-functions, for an R4 definition
-- Input: an R4 definition
-- Output: an R4 definition
-- Moves arguments > 6 into a vector
limitDefn :: TypedR4Definition -> TypedR4Definition
limitDefn (DefnT name args outputT body) = undefined

-- Limit-functions, for a list of R4 definitions
-- Input: a list of R4 definitions
-- Output: a list of R4 definitions
-- Moves arguments > 6 into a vector
limitFunctions :: [TypedR4Definition] -> [TypedR4Definition]
...
type Binding = (Variable, TypedR4Expr)

-- Make a "LET" expression from a list of bindings and a final "body" expression
mkLet :: [Binding] -> TypedR4Expr -> TypedR4Expr
mkLet [] body = body
mkLet ((x, e) : bs) body = LetTE x e (mkLet bs body)

-- Expose allocation, for an R4 expression
-- Input: an R4 expression
-- Output: an R4 expression, without "VectorTE" expressions
-- This pass compiles "VectorTE" expressions into explicit allocations
eaExp :: TypedR4Expr -> TypedR4Expr
eaExp e = undefined

-- Expose allocation, for an R4 definition
-- Input: an R4 definition
-- Output: an R4 definition, without "VectorTE" expressions
eaDefn :: TypedR4Definition -> TypedR4Definition
...
-- Input: an R4 expression
-- Output: an R4 expression, without "VectorTE" expressions
exposeAllocation :: [TypedR4Definition] -> [TypedR4Definition]
...
rcoExp :: TypedR4Expr -> TypedR4Expr
rcoExp e = undefined
...
rcoArg :: TypedR4Expr -> (TypedR4Expr, [Binding])
rcoArg e = undefined

-- Remove complex operands, for an R4 definition
-- Input: an R4 definition
-- Output: an R4 definition, without complex operands
rcoDefn :: TypedR4Definition -> TypedR4Definition
rcoDefn (DefnT name args outputT body) =
  DefnT name args outputT (rcoExp body)

-- Remove complex operands, for an R4 program
-- Input: an R4 program
-- Output: an R4 program, without complex operands
removeComplexOpera :: [TypedR4Definition] -> [TypedR4Definition]
...
-- Compile a R4 argument (integer or variable) into a C3Arg expression
ecArg :: TypedR4Expr -> C3Arg
ecArg e = case e of
  IntTE i -> IntC3 i
  VarTE x t -> VarC3 x t
  TrueTE -> TrueC3
...
-- Compile a BASIC R4 Expression into a C3Basic Expression
ecBasic :: TypedR4Expr -> C3Basic
ecBasic e = undefined
...
--   - e: the R4 Expression it is being assigned to
--   - k: a C3 Tail expression describing what should happen *after* the assignment
--   - cfg: a C3CFG (control flow graph)
-- output: a C3 Tail expression
-- Sometimes updates 'cfg' with new CFG nodes
ecAssign :: Variable -> TypedR4Expr -> C3Tail -> C3CFG -> C3Tail
ecAssign x e k cfg = undefined

ecDefn :: TypedR4Definition -> C3Defn
ecDefn (DefnT name args outputT body) = undefined

-- The explicate control pass
-- input: an R4 expression
-- output: a C3 control flow graph
explicateControl :: [TypedR4Definition] -> [C3Defn]
...
rootStackInit name rootStackSpills = undefined
...
compileLog :: R4Program -> IO String
compileLog e =
