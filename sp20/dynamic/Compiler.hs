...
-- Compile R7 to R6
compR7Expr :: R7Expr -> R6Expr
compR7Expr e = undefined

compR7Defn :: R7Definition -> R6Definition
compR7Defn (DefnD name args body) =
  let args' = map (\a -> (a, AnyT)) args
  in (Defn name args' AnyT (compR7Expr body))

compileR7 :: R7Program -> R6Program
compileR7 (defns, e) =
  let defns' = map compR7Defn defns
      e'     = compR7Expr e
  in (defns', e')
...
-- Typechecker for R6 programs
typecheck :: R6Program -> TypedR6Program
typecheck (defns, e) =
...
-- Shrink, for R6 expressions
-- Input: e, an R6 expression
-- Output: an R6 expression
-- Removes "and", "or", ">=", ">", ">="
shrinkExpr :: TypedR6Expr -> TypedR6Expr
shrinkExpr e = undefined

-- Get the tag associated with a type
tagOf :: Type -> Int
tagOf IntT        = 1
tagOf BoolT       = 4
tagOf (VectorT _) = 2
...
-- The shrink pass, for an R6 definition
-- Input:  an R6 definition
-- Output: an R6 definition
shrinkDefn :: TypedR6Definition -> TypedR6Definition
shrinkDefn (DefnT name args outputT body) =
  (DefnT name args outputT (shrinkExpr body))

-- The shrink pass, for an R6 program
-- Input:  an R6 program
-- Output: a list of R6 definitions
-- Moves the top-level expression into a "main" definition
shrink :: TypedR6Program -> [TypedR6Definition]
shrink (defns, e) =
...
-- Output: an R6 expression, with variables renamed to be unique
uniquifyExp :: TypedR6Expr -> VEnv -> TypedR6Expr
uniquifyExp e env = undefined

-- The uniquify pass, for a single R6 definition
-- Input:  an R6 definition
-- Output: an R6 definition, with variables renamed to be unique
uniquifyDefn :: VEnv -> TypedR6Definition -> TypedR6Definition
...
-- The uniquify pass, for a list of R6 definitions
-- Input:  a list of R6 definitions
-- Output: a list of R6 definitions, with variables renamed to be unique
uniquify :: [TypedR6Definition] -> [TypedR6Definition]
uniquify defns =
...
-- Reveal-functions, for R6 expressions
-- Input: e, an R6 expression
-- Output: an R6 expression
-- Transforms variables referencing function names with FunRefTE expressions
revealExpr :: TypedR6Expr -> [String] -> TypedR6Expr
...
-- Output: an R6 expression
-- Transforms variables referencing function names with FunRefTE expressions
revealDefn :: [String] -> TypedR6Definition -> TypedR6Definition
revealDefn funs (DefnT name args outputT body) =
  (DefnT name args outputT (revealExpr body funs))

-- Reveal-functions, for a list of R6 definitions
-- Input: a list of R6 definitions
-- Output: a list of R6 definitions
-- Transforms variables referencing function names with FunRefTE expressions
revealFunctions :: [TypedR6Definition] -> [TypedR6Definition]
...
freeVars :: TypedR6Expr -> Set TypedVariable
freeVars e = undefined

-- Closure conversion, for R6 expressions
-- Input: e, an R6 expression
-- Output: a pair
--  - an R6 expression
--  - a list of new top-level definitions (from closure conversion)
cconvExpr :: TypedR6Expr -> (TypedR6Expr, [TypedR6Definition])
...
mkClosureDefn :: String -> [TypedVariable] -> [TypedVariable] ->
  TypedR6Expr -> Type -> TypedR6Definition
...
-- Closure conversion, for R6 definitions
-- Input: an R6 definition
-- Output: an R6 definition
cconvDefn :: TypedR6Definition -> [TypedR6Definition]
...
-- Closure conversion, for a list of R6 definitions
-- Input: a list of R6 definitions
-- Output: a list of R6 definitions
closureConversion :: [TypedR6Definition] -> [TypedR6Definition]
...
type EEnv = [(Variable, TypedR6Expr)]

-- Limit-functions, for R6 expressions
-- Input:
--  - e, an R6 expression
--  - env, an environment
-- Output: an R6 expression
-- Moves arguments > 6 into a vector
limitExpr :: TypedR6Expr -> EEnv -> TypedR6Expr
limitExpr e env = undefined

-- Limit-functions, for an R6 definition
-- Input: an R6 definition
-- Output: an R6 definition
-- Moves arguments > 6 into a vector
limitDefn :: TypedR6Definition -> TypedR6Definition
limitDefn (DefnT name args outputT body) = undefined

-- Limit-functions, for a list of R6 definitions
-- Input: a list of R6 definitions
-- Output: a list of R6 definitions
-- Moves arguments > 6 into a vector
limitFunctions :: [TypedR6Definition] -> [TypedR6Definition]
...
type Binding = (Variable, TypedR6Expr)

-- Make a "LET" expression from a list of bindings and a final "body" expression
mkLet :: [Binding] -> TypedR6Expr -> TypedR6Expr
mkLet [] body = body
mkLet ((x, e) : bs) body = LetTE x e (mkLet bs body)

-- Expose allocation, for an R6 expression
-- Input: an R6 expression
-- Output: an R6 expression, without "VectorTE" expressions
-- This pass compiles "VectorTE" expressions into explicit allocations
eaExp :: TypedR6Expr -> TypedR6Expr
eaExp e = undefined

-- Expose allocation, for an R6 definition
-- Input: an R6 definition
-- Output: an R6 definition, without "VectorTE" expressions
eaDefn :: TypedR6Definition -> TypedR6Definition
...
-- Input: an R6 expression
-- Output: an R6 expression, without "VectorTE" expressions
exposeAllocation :: [TypedR6Definition] -> [TypedR6Definition]
...
rcoExp :: TypedR6Expr -> TypedR6Expr
rcoExp e = undefined
...
rcoArg :: TypedR6Expr -> (TypedR6Expr, [Binding])
rcoArg e = undefined

-- Remove complex operands, for an R6 definition
-- Input: an R6 definition
-- Output: an R6 definition, without complex operands
rcoDefn :: TypedR6Definition -> TypedR6Definition
rcoDefn (DefnT name args outputT body) =
  DefnT name args outputT (rcoExp body)

-- Remove complex operands, for an R6 program
-- Input: an R6 program
-- Output: an R6 program, without complex operands
removeComplexOpera :: [TypedR6Definition] -> [TypedR6Definition]
...
data C3Basic = C3ArgE C3Arg
             ...
             | C3CallE C3Arg [C3Arg]
             | C3InjectE C3Arg Type
             | C3TagOfAnyE C3Arg
             | C3ValueOfAnyE C3Arg Type
             | C3ExitE
  deriving (Eq, Ord, Show)
...
-- Compile a R6 argument (integer or variable) into a C3Arg expression
ecArg :: TypedR6Expr -> C3Arg
ecArg e = case e of
  IntTE i -> IntC3 i
  VarTE x t -> VarC3 x t
  TrueTE -> TrueC3
...
-- Compile a BASIC R6 Expression into a C3Basic Expression
ecBasic :: TypedR6Expr -> C3Basic
ecBasic e = undefined
...
--   - e: the R6 Expression it is being assigned to
--   - k: a C3 Tail expression describing what should happen *after* the assignment
--   - cfg: a C3CFG (control flow graph)
-- output: a C3 Tail expression
-- Sometimes updates 'cfg' with new CFG nodes
ecAssign :: Variable -> TypedR6Expr -> C3Tail -> C3CFG -> C3Tail
ecAssign x e k cfg = undefined


ecDefn :: TypedR6Definition -> C3Defn
...
explicateControl :: [TypedR6Definition] -> [C3Defn]
...
compile = printX86 . patchInstructions . allocateRegisters . buildInterference .
  ...
  limitFunctions . closureConversion . revealFunctions . uniquify . shrink . typecheck .
  compileR7
...
compileLog :: R7Program -> IO String
compileLog e =
  (logOutput "input" id) e >>=
  (logOutput "compile-R7" compileR7) >>=
