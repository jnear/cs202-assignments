...
_rfun_parser = Lark(r"""
    ?exp: NUMBER -> int_e
        | "BoundInt" "<" NUMBER ">" "(" NUMBER ")" -> bound_int_e
        | "True" -> true_e
        | "False" -> false_e
        | CNAME -> var_e
...
def _parse(s: str) -> RfunProgram:
    ...
    def btype(t):
        ...
        elif t.data == 'bound_int_t':
            assert len(t.children) == 1
            return BoundIntT(int(t.children[0]))
        else:
    ...
    def bast(e) -> RfunExp:
        ...
        elif e.data == 'bound_int_e':
            upper_bound, int_val = e.children
            return BoundInt(int(upper_bound), int(int_val))
        else:
